...
<script>
  import progressModel from '@/views/page/bigView/left/echarts/progressModel'
      ...
      */
      ...
      getoutput_forward_day() {
        getoutput_forward_day().then(data => {
          if (data && data.success) {
            if (data.data.length) {
              this.value1 = parseInt(data.data[0].日产量)
              this.value2 = parseInt(data.data[0].日掘进进尺)
            }
          }
        })
      },
      ...
      getwashing_day() {
        getwashing_day().then(data => {
          if (data && data.success) {
            if (data.data.length) {
              this.value3 = parseInt(data.data[0].日洗煤量)
            }
          }
        })
      },
      ...
      getdrilling_day() {
        getdrilling_day().then(data => {
          if (data && data.success) {
            if (data.data.length) {
              this.value4 = parseInt(data.data[0].日抽放打钻进尺)
            }
          }
        })
      },
      ...
      getselling_day() {
        getselling_day().then(data => {
          if (data && data.success) {
            if (data.data.length) {
              this.value5 = parseInt(data.data[0].日销量)
            }
          }
        })
      },
